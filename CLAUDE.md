...
- `webui.py` — interface Flask locale (127.0.0.1:8765). `create_app(root=None)` injectable (tous les chemins dérivés de `root`, testable via test_client). Login par membre (session, `before_request` → 401 sur `/api/*`), secret persisté dans `data/secret_key` (0600). Onglets historiques (liens/tracks/comptes/plan/réglages) + Niches et Presets (CRUD + upload clip + liens/téléchargement vidéo par niche). Défense XSS : `esc()` sur tout champ STATE rendu via innerHTML + coercion serveur des champs numériques (cadence via `db.update_niche`, overrides via `coerce_overrides`) → 400 si non convertible. Parties pures testées : `plan_to_toml`. `merge_settings`/`load_settings` vivent dans beatsync.
